...
def test_all_equal_nufft1d2():
    xj = np.random.random_sample(NJ)
    fk = np.random.random_sample(MS).astype(np.complex128)
...
def test_all_equal_numba_nufft3d1():
    ...
    fk1 = njit_nufft3d1(xj, yj, zj, cj, MS, MT, MU, df=1, eps=EPS, iflag=IFLAG)

    # Reshape and convert from fortran ordered
    fk1 = fk1.view(np.complex128).reshape((MS, MT, MU), order="F")
...
def test_all_equal_nufft3d2():
    ...
    ier = cff_nufft.lib.nufft3d2(
        ...
        MT,
        MS,
        cffi_support.ffi.cast("double *", fk1.ctypes.data),
    )

    cj2 = f2p_nufft.nufft3d2(xj, yj, zj, fk, eps=EPS, iflag=IFLAG)
