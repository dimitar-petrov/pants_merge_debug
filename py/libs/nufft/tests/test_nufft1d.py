...
def _get_data_roundtrip():
    ...
    x = np.sort(
        np.random.choice(
            np.linspace(-np.pi, np.pi, ms, endpoint=False), nj, replace=False
        )
    )
...
class NUFFT1DTestCase(unittest.TestCase):
    ...
    def _type_1_even(self, eps=1e-10):
        ...
        self.assertEqual(
            len(nufft1d1freqs(len(self.f))), len(p1), "Wrong length of frequency array"
        )
    ...
    def _type_1_odd(self, eps=1e-10):
        ...
        self.assertEqual(
            len(nufft1d1freqs(len(self.f) + 1)),
            len(p1),
    ...
    def _type_1_and_3(self, eps=1e-10):
        ...
        self.assertTrue(_error(p1, p2) < eps, "Type 1 and 3 and not close (even)")

        f = nufft1d1freqs(len(f) + 1)
        p2 = nufft1d3(self.x, self.c, f, eps=eps)
        p1 = nufft1d1(self.x, self.c, len(f), eps=eps)
        self.assertTrue(_error(p1, p2) < eps, "Type 1 and 3 and not close (odd)")
