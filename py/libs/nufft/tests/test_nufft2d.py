...
class NUFFT2DTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Coordinates
        x = [
            2 * np.pi * np.arange(self.N[0]) / self.N[0],
        ...
        # Frequency points
        self.st_grid = np.meshgrid(
            np.arange(self.N[0]), np.arange(self.N[1]), indexing="ij"
        )
    ...
    def _type_1_even(self, eps=1e-10):
        ...
        self.assertTrue(
            _error(p1, p2) < eps, "Type 1: Discrepancy between direct and fft function"
        )
    ...
    def _type_2(self, eps=1e-10):
        c2 = nufft2d2(
            self.X[0].reshape(-1),
            self.X[1].reshape(-1),
            np.roll(np.roll(self.c, -int(self.N[0] / 2), 0), -int(self.N[1] / 2), 1),
            direct=True,
        )
        c1 = nufft2d2(
            self.X[0].reshape(-1),
        ...
        self.assertTrue(
            _error(c1, c2) < eps, "Type 2: Discrepancy between direct and fft function"
        )
    ...
    def _type_3(self, eps=1e-10):
        ...
        self.assertTrue(
            _error(p1, p2) < eps, "Type 3: Discrepancy between direct and fft function"
        )
    ...
    def _type_1_2_roundtrip(self, eps=1e-10):
        ...
        c2 = nufft2d2(
            self.X[0].reshape(-1), self.X[1].reshape(-1), p, iflag=1, direct=True
        )
        self.assertTrue(
            _error(self.c.reshape(-1), c2) < eps, "Type 1 and 2: roundtrip error."
        )
    ...
    def _type_1_and_3(self, eps=1e-10):
        ...
        self.assertTrue(_error(p1.reshape(-1), p2) < eps, "Type 1 and 3 and not close")
    ...
    def test_type1_dft(self):
        ...
        self.assertTrue(
            _error(self.f_numpy.reshape(-1), f_dir1.reshape(-1)) < self.eps,
            "NUFFT direct DFT (1) vs. NumPy FFT: error too large",
        )
        self.assertTrue(
            _error(self.f_numpy.reshape(-1), f_nufft1.reshape(-1)) < self.eps,
    ...
    def test_type1_irdft(self):
        ...
        K = np.meshgrid(-self.kx, -self.ky[int(self.N[1] / 2) + 1 :], indexing="ij")
        f = np.concatenate((self.fr_numpy, np.conj(self.fr_numpy[K[0], K[1]])), axis=1)
    ...
    def test_type2_dft(self):
        ...
        self.assertTrue(
            _error(self.f_numpy.reshape(-1), f_dir2) < self.eps,
            "NUFFT direct DFT (2) vs. NumPy FFT: error too large",
        )
        self.assertTrue(
            _error(self.f_numpy.reshape(-1), f_nufft2) < self.eps,
    ...
    def test_type2_idft(self):
        ...
        c_dir2 = (
            nufft2d2(
                ...
                self.X[1].reshape(-1),
                np.roll(
                    np.roll(self.f_numpy, -int(self.N[0] / 2), 0),
                    -int(self.N[1] / 2),
                    1,
                ),
                iflag=1,
        ...
        self.assertTrue(
            _error(self.c_numpy.reshape(-1), c_dir2) < self.eps,
            "NUFFT direct IDFT (2) vs. NumPy IFFT: error too large",
        )
        self.assertTrue(
            _error(self.c_numpy.reshape(-1), c_nufft2) < self.eps,
    ...
    def test_type3_dft(self):
        ...
        self.assertTrue(
            _error(self.f_numpy.reshape(-1), f_dir3) < self.eps,
            "NUFFT direct DFT (3) vs. NumPy FFT: error too large",
        )
        self.assertTrue(
            _error(self.f_numpy.reshape(-1), f_nufft3) < self.eps,
    ...
    def test_type3_idft(self):
        ...
        self.assertTrue(
            _error(self.c_numpy.reshape(-1), c_dir3) < self.eps,
            "NUFFT direct IDFT (2) vs. NumPy IFFT: error too large",
        )
        self.assertTrue(
            _error(self.c_numpy.reshape(-1), c_nufft3) < self.eps,
