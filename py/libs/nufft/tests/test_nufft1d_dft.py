...
class NUFFT1DTestCase(unittest.TestCase):
    ...
    def test_type1_dft(self):
        ...
        self.assertTrue(
            _error(self.f_numpy, f_dir1) < self.eps,
            "NUFFT direct DFT (1) vs. NumPy FFT: error too large",
        )
        self.assertTrue(
            _error(self.f_numpy, f_nufft1) < self.eps,
            "NUFFT direct DFT (1) vs. NumPy FFT: error too large",
        )
    ...
    def test_type1_irdft(self):
        ...
        c_dir = np.roll(
            nufft1d1(self.x, f, self.N, iflag=1, direct=True), -int(self.N / 2), 0
        )
        c_nufft = np.roll(
            nufft1d1(self.x, f, self.N, iflag=1, direct=False), -int(self.N / 2), 0
        )
    ...
    def test_type2_dft(self):
        ...
        f_dir2 = nufft1d2(
            self.x, np.roll(self.c, -int(self.N / 2), 0), iflag=-1, direct=True
        )
        f_nufft2 = nufft1d2(
            self.x, np.roll(self.c, -int(self.N / 2), 0), iflag=-1, direct=False
        )

        self.assertTrue(
            _error(self.f_numpy, f_dir2) < self.eps,
            "NUFFT direct DFT (2) vs. NumPy FFT: error too large",
        )
        self.assertTrue(
            _error(self.f_numpy, f_nufft2) < self.eps,
    ...
    def test_type2_idft(self):
        ...
        c_dir2 = (
            nufft1d2(
                self.x, np.roll(self.f_numpy, -int(self.N / 2), 0), iflag=1, direct=True
            )
            / self.N
        )
        c_nufft2 = (
            nufft1d2(
                self.x,
                np.roll(self.f_numpy, -int(self.N / 2), 0),
                iflag=1,
                direct=False,
            )
            / self.N
        ...
        self.assertTrue(
            _error(self.c_numpy, c_dir2) < self.eps,
            "NUFFT direct IDFT (2) vs. NumPy IFFT: error too large",
        )
        self.assertTrue(
            _error(self.c_numpy, c_nufft2) < self.eps,
            "NUFFT IFFT (2) vs. NumPy IFFT: error too large",
        )

    def test_type3_dft(self):
        """Is the NUFFT type 3 DFT correct?"""
        f_dir3 = nufft1d3(self.x, self.c, self.st, iflag=-1, direct=True) * self.N
        f_nufft3 = nufft1d3(self.x, self.c, self.st, iflag=-1, direct=False) * self.N

        self.assertTrue(
            _error(self.f_numpy, f_dir3) < self.eps,
            "NUFFT direct DFT (3) vs. NumPy FFT: error too large",
        )
        self.assertTrue(
            _error(self.f_numpy, f_nufft3) < self.eps,
            "NUFFT FFT (3) vs. NumPy FFT: error too large",
        )

    def test_type3_idft(self):
        """Is the NUFFT type 3 IDFT correct?"""
        c_dir3 = nufft1d3(self.x, self.f_numpy, self.st, iflag=1, direct=True)
        c_nufft3 = nufft1d3(self.x, self.f_numpy, self.st, iflag=1, direct=False)

        self.assertTrue(
            _error(self.c_numpy, c_dir3) < self.eps,
            "NUFFT direct IDFT (2) vs. NumPy IFFT: error too large",
        )
        self.assertTrue(
            _error(self.c_numpy, c_nufft3) < self.eps,
