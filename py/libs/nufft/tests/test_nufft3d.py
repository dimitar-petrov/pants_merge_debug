...
class NUFFT3DTestCase(unittest.TestCase):
    ...
    def _type_2(self, eps=1e-10):
        c2 = nufft3d2(
            ...
            np.roll(
                np.roll(
                    np.roll(self.c, -int(self.N[0] / 2), 0), -int(self.N[1] / 2), 1
                ),
                -int(self.N[2] / 2),
                2,
            ),
    ...
    def test_type1_irdft(self):
        ...
        K = np.meshgrid(
            -self.kx, -self.ky, -self.kz[int(self.N[2] / 2) + 1 :], indexing="ij"
        )
        f = np.concatenate(
            (self.fr_numpy, np.conj(self.fr_numpy[K[0], K[1], K[2]])), axis=2
        )
